...
IValue Unpickler2::parse_ivalue() {
  run();
  TORCH_CHECK(stack_.size() == 1,
...
double Unpickler2::readFloat() {
  ...
  std::reverse_copy(big_endian_ptr, big_endian_ptr + sizeof(big_endian),
...
void Unpickler2::run() {
  ...
  TORCH_CHECK(opcode == PickleOpCode::PROTO,
  ...
  TORCH_CHECK(
      protocol == 2,
      "Only Pickle protocol 2 is supported, found protocol = ", protocol);
...
void Unpickler2::setInput(size_t memo_id) {
  AT_ASSERT(!stack_.empty());
  if (memo_id >= memo_table_.size()) {
    memo_table_.insert(memo_table_.end(), memo_id - memo_table_.size(),
                       IValue());
...
template <typename T> inline void append(std::vector<T> &a, T &&e) {
  a.emplace_back(std::move(e));
}
template <> inline void append<bool>(std::vector<bool> &a, bool &&e) {
  a.push_back(e);
}

static std::vector<int64_t> tupleToIntList(const IValue &v) {
  return fmap(v.toTuple()->elements(),
              [](const IValue &v) -> int64_t { return v.toInt(); });
}

// note we cannot use toIntList, toDoubleList because during unpickling the
// lists are not yet tagged
template <typename T> static std::vector<T> convertList(const IValue &v) {
  return fmap(v.toListRef(), [](const IValue &elem) { return elem.to<T>(); });
}

PickleOpCode Unpickler2::readInstruction() {
  auto opcode = readOpCode();
  switch (opcode) {
  ...
  case PickleOpCode::LONG_BINPUT: {
    TORCH_CHECK(std::numeric_limits<size_t>::max() >=
  ...
  case PickleOpCode::EMPTY_DICT:
    stack_.emplace_back(c10::impl::GenericDict(AnyType::get(), AnyType::get()));
    break;
  ...
  case PickleOpCode::BINPERSID: {
    ...
    AT_ASSERT(args.at(0).toStringRef() == "storage", "unknown PERSID key ",
    ...
    at::Storage storage(c10::Storage::use_byte_size_t(),
                        numel * dtype.itemsize(), std::move(storage_ptr),
                        /*allocator=*/nullptr,
                        /*resizable=*/false); // NB: we didn't set any allocator
                                              // for the tensor
  ...
  default: {
    AT_ERROR("Unknown opcode for unpickling at ",
...
void Unpickler2::readGlobal(const std::string &module_name,
  ...
  if (module_name == "__main__") {
    if (class_name == "TensorID") {
      globals_.emplace_back([this] {
        ...
        stack_.pop_back();
        TORCH_INTERNAL_ASSERT(tensor_table_,
      ...
      globals_.emplace_back([this] {
        ...
        if (entry != type_cache_.end()) {
          ...
          if (type_resolver_ == nullptr) {
            ...
            // type = c10::parseType(type_str); // by me
          } else {
...
void Unpickler2::rebuildTensor(bool quantized) {
  globals_.emplace_back([this, quantized] {
    ...
    if (quantized) {
      ...
      case at::kPerTensorAffine: {
        ...
        result = at::_empty_affine_quantized({0}, storage_tensor.options(),
...
#ifdef USE_RPC
void Unpickler2::rebuildRRef() {
  globals_.emplace_back([this] {
    // It is the same as how rref is unpickled in python,
    // see PyRRef::unpickle
    auto args = stack_.back().toTuple()->elements();
    stack_.pop_back();
    TORCH_INTERNAL_ASSERT(args.size() == distributed::rpc::RFD_TUPLE_SIZE,
    ...
    auto rrefForkData = distributed::rpc::RRefForkData(ownerId, rrefId, forkId,
                                                       parent, typeStr);
    auto &ctx = distributed::rpc::RRefContext::getInstance();
    c10::intrusive_ptr<distributed::rpc::RRef> rref;
    TORCH_INTERNAL_ASSERT(type_resolver_ != nullptr,
                          "type_resolver_ is nullptr.");
    at::StrongTypePtr type = type_resolver_(c10::QualifiedName(typeStr));
    rref = ctx.getOrCreateRRef(rrefForkData, type.type_);
    ctx.notifyOwnerAndParentOfFork(rrefForkData.forkId_, rrefForkData.parent_,
                                   rref);
    stack_.emplace_back(
...
void Unpickler2::readSlowWithBuffer(char *dest, size_t sz) {
...
std::string Unpickler2::readBytes(size_t length) {
  std::string data;
...
void Unpickler2::readList(IValue list_ivalue) {
  size_t start = marks_.back();
  marks_.pop_back();
...
std::string Unpickler2::readString() {
  std::string ss;
  while (true) {
    char c = read<char>();
    if (c == '\n') {
    ...
    TORCH_CHECK(is_valid_python_id_char(c), "Found character '",
                int(uint8_t(c)), "' in string, ",
