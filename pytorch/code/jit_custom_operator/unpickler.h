...
class TORCH_API Unpickler2 {
  TH_DISALLOW_COPY_AND_ASSIGN(Unpickler2);
 ...
 public:
  ...
  Unpickler2(std::function<size_t(char *, size_t)> reader,
  ...
  Unpickler2(std::function<size_t(char *, size_t)> reader,
             TypeResolver type_resolver, ObjLoader obj_loader,
  ...
  void set_version(uint64_t version_number) { version_ = version_number; }
 ...
 public:
  ...
  double readFloat();
  void readGlobal(const std::string &module_name,
